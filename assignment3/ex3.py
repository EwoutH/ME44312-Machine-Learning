...
# Using the elbow method to decide on the number of clusters
...
for i in range(1, N_max):
    # Cluster using k-means. Use 25 different initializations to avoid local minima
    kmeans = KMeans(n_clusters = i, n_init=25, random_state=1)
    kmeans.fit(X)
...
# NOTE have random_state=1 in order to control randomness of the process for checking the results]\
n_clusters = 3
kmeans = KMeans(n_clusters=n_clusters, n_init=25, random_state=1)
kmeans.fit(X)

# you can check kmeans.inertia_ to see the error (the sum of squared distances)
print(f"Inertia (error): {kmeans.inertia_}")

# Add a column for the clusters to the original X dataframe in order to be able to analyze the results based on original values
X['cluster'] = kmeans.labels_
org_X['cluster'] = kmeans.labels_

# END OF PART C #


# What percentage of the data is assigned to each cluster?
print("Percentage of the data assigned to each cluster:")
for i in range(n_clusters):
    print(f"Cluster {i+1}: {len(X[X['cluster'] == i])/len(X):.2%}")

# The average trip duration in the cluster
print("\nAverage trip durations in the cluster:")
for i in range(n_clusters):
    print(f"Cluster {i+1}: {org_X[org_X['cluster'] == i]['Duration'].mean():.4}")

# The percentage of trips made by people above 50 years old in the cluster
print("\nPercentage of trips made by people above 50 years old in the cluster:")
for i in range(n_clusters):
    print(f"Cluster {i+1}: {len(org_X[(org_X['cluster'] == i) & (org_X['Age'] > 50)])/len(org_X[org_X['cluster'] == i]):.2%}")

# The percentage of trips made in the evening in the cluster
print("\nPercentage of trips made in the evening in the cluster:")
for i in range(3):
    print(f"Cluster {i+1}: {len(org_X[(org_X['cluster'] == i) & (org_X['Evening'] == 1)])/len(org_X[org_X['cluster'] == i]):.2%}")
